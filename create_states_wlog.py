...
edge_distributions = ['real-normal']
ed = edge_distributions[0]
offset = 1
reps = 2
anneal = True
...
for year in years:
    ...
    #Set new edge property, wlog = log(w)+offset
    eprop_wlog = g.new_edge_property("double")
    for e in g.edges():
        eprop_wlog[e] = np.log(g.edge_properties['weight'][e])+offset
    g.edge_properties['wlog'] = eprop_wlog

    t1 = time()

    #initial state computation
    stateW = gt.minimize_nested_blockmodel_dl(
        g, state_args=dict(recs=[g.ep.wlog], rec_types=[ed]))
    ...
    if write_all_states:
        ...
        outfile = outfname+str(year)+'stateW.p'
